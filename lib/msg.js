...
const downloadMediaMessage = async (m, filename) => {
    ...
    switch (m.type) {
        ...
        case 'documentMessage': {
            let ext = m.msg.fileName.split('.').pop().toLowerCase();
...
const sms = (malvin, m, store) => {
    ...
    // Safely extract content and context
    m.mtype = m.message ? getContentType(m.message) : null;
    m.msg = m.message && m.mtype ? (m.mtype === 'viewOnceMessage'
    ...
    // Safely get quoted message and mentions
    ...
    m.reply = async (content, opt = { packname: "Secktor", author: "SamPandey001" }, type = "text") => {
        ...
        switch (type.toLowerCase()) {
            ...
            case 'image':
                return await malvin.sendMessage(m.chat, Buffer.isBuffer(content) ? { image: content, ...opt } : { image: { url: content }, ...opt }, { quoted: m });
            case 'video':
                return await malvin.sendMessage(m.chat, Buffer.isBuffer(content) ? { video: content, ...opt } : { video: { url: content }, ...opt }, { quoted: m });
            case 'audio':
                return await malvin.sendMessage(m.chat, Buffer.isBuffer(content) ? { audio: content, ...opt } : { audio: { url: content }, ...opt }, { quoted: m });
        }
    };
